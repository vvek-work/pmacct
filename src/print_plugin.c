...
void print_plugin(int pipe_fd, struct configuration *cfgptr, void *ptr) {
    ...
    if (config.print_output != PRINT_OUTPUT_JSON &&
        ...
        if (config.tcpflags_encode_as_array ||
            ...
            Log(LOG_WARNING,
                "WARN ( %s/%s ): Complex data types (ie. pre_tag_label_encode_as_map) not supported by the selected print_output. Ignored.\n",
    ...
    if (config.print_output & PRINT_OUTPUT_JSON) {
        ...
        if (config.print_output_custom_lib != NULL) {
            custom_output_setup(config.print_output_custom_lib, config.print_output_custom_cfg_file,
                                &custom_print_plugin);
        }
    }
    ...
    P_init_refresh_deadline(&refresh_deadline, config.sql_refresh_time, config.sql_startup_delay,
                            config.sql_history_roundoff);
    ...
    if ((extras.off_pkt_vlen_hdr_primitives || config.what_to_count_2 & COUNT_MPLS_LABEL_STACK) &&
        ...
        Log(LOG_ERR,
            "ERROR ( %s/%s ): variable-length primitives, ie. label as_path std_comm etc., are not supported in print plugin with formatted output.\n",
            config.name, config.type);
        Log(LOG_ERR,
            "ERROR ( %s/%s ): Please switch to one of the other supported output formats (ie. csv, json, avro). Exiting ..\n",
            config.name, config.type);
        exit_gracefully(1);
    }
    ...
    if (!config.sql_table && config.daemon) {
        Log(LOG_ERR,
            "ERROR ( %s/%s ): no print_output_file defined and 'daemonize: true'. Output would be lost. Exiting ..\n",
            config.name, config.type);
        exit_gracefully(1);
    }
    ...
    if (config.sql_table) {
        if (strchr(config.sql_table, '%') || strchr(config.sql_table, '$')) {
            ...
            if (config.print_latest_file &&
                (strchr(config.print_latest_file, '%') || strchr(config.print_latest_file, '$'))) {
                    ...
                    if (((struct ch_buf_hdr *) rg->ptr)->seq != seq) {
                        if (!pollagain) {
                            ...
                            if (config.debug || (rg_err_count > MAX_RG_COUNT_ERR)) {
                                Log(LOG_WARNING,
                                    "WARN ( %s/%s ): Missing data detected (plugin_buffer_size=%" PRIu64 " plugin_pipe_size=%" PRIu64 ").\n",
                                    config.name, config.type, config.buffer_size, config.pipe_size);
                                Log(LOG_WARNING,
                                    "WARN ( %s/%s ): Increase values or look for plugin_buffer_size, plugin_pipe_size in CONFIG-KEYS document.\n\n",
                                    config.name, config.type);
                            }
...
void P_cache_purge(struct chained_cache *queue[], int index, int safe_action) {
        ...
        if (config.print_output_lock_file) {
            ...
            if (!lockf)
                Log(LOG_WARNING, "WARN ( %s/%s ): Failed locking print_output_lock_file: %s\n", config.name,
                    config.type, config.print_output_lock_file);
        }
    ...
    for (j = 0; j < index; j++) {
        ...
        if (dyn_table && (!dyn_table_time_only || !config.nfacctd_time_new || (config.sql_refresh_time != timeslot))) {
            ...
            handle_dynname_internal_strings(elem_table, SRVBUFLEN, config.sql_table, &elem_prim_ptrs,
                                            DYN_STR_PRINT_FILE);
        ...
        if (!go_to_pending) {
            ...
            if (f && config.print_output & PRINT_OUTPUT_FORMATTED) {
                ...
                if (config.what_to_count & COUNT_CLASS)
                    fprintf(f, "%-16s  ", ((data->class && class[(data->class) - 1].id) ? class[(data->class) -
                                                                                                1].protocol
                                                                                        : "unknown"));
#if defined (WITH_NDPI)
                ...
                if (config.what_to_count_2 & COUNT_SAMPLING_DIRECTION)
                    fprintf(f, "%-1s                   ", sampling_direction_print(data->sampling_direction));
                ...
                if (config.what_to_count_2 & COUNT_POST_NAT_SRC_PORT)
                    fprintf(f, "%-5u              ", pnat->post_nat_src_port);
                if (config.what_to_count_2 & COUNT_POST_NAT_DST_PORT)
                    fprintf(f, "%-5u              ", pnat->post_nat_dst_port);
                ...
                if (config.what_to_count_2 & COUNT_TUNNEL_SRC_PORT)
                    fprintf(f, "%-5u            ", ptun->tunnel_src_port);
                if (config.what_to_count_2 & COUNT_TUNNEL_DST_PORT)
                    fprintf(f, "%-5u            ", ptun->tunnel_dst_port);
                if (config.what_to_count_2 & COUNT_TUNNEL_TCPFLAGS)
                    fprintf(f, "%-3u               ", queue[j]->tunnel_tcp_flags);
                ...
                if (config.what_to_count_2 & COUNT_EXPORT_PROTO_VERSION)
                    fprintf(f, "%-20u  ", data->export_proto_version);
                ...
                /* all custom primitives printed here */
                {
                    ...
                    for (cp_idx = 0; cp_idx < config.cpptrs.num; cp_idx++) {
                        if (config.cpptrs.primitive[cp_idx].ptr->len != PM_VARIABLE_LENGTH) {
                            ...
                            custom_primitive_value_print(cp_str, SRVBUFLEN, pcust, &config.cpptrs.primitive[cp_idx],
                                                         TRUE);
                            fprintf(f, "%s  ", cp_str);
                        } else {
                ...
                if (config.what_to_count_2 & COUNT_LABEL)
                    P_fprintf_csv_string(f, pvlen, COUNT_INT_LABEL, write_sep(sep, &count), empty_string);
                if (config.what_to_count & COUNT_CLASS)
                    fprintf(f, "%s%s", write_sep(sep, &count), ((data->class && class[(data->class) - 1].id) ? class[
                            (data->class) - 1].protocol : "unknown"));
#if defined (WITH_NDPI)
                if (config.what_to_count_2 & COUNT_NDPI_CLASS) {
              snprintf(ndpi_class, SUPERSHORTBUFLEN, "%s/%s",
                ndpi_get_proto_name(pm_ndpi_wfl->ndpi_struct, data->ndpi_class.master_protocol),
              ...
              fprintf(f, "%s%s", write_sep(sep, &count), ndpi_class);
                ...
                if (config.what_to_count_2 & COUNT_OUT_VLAN)
                    fprintf(f, "%s%u", write_sep(sep, &count), data->out_vlan_id);
                ...
                if (config.what_to_count & (COUNT_SRC_AS | COUNT_SUM_AS))
                    fprintf(f, "%s%u", write_sep(sep, &count), data->src_as);
                ...
                if (config.what_to_count & COUNT_LOCAL_PREF)
                    fprintf(f, "%s%u", write_sep(sep, &count), pbgp->local_pref);
                if (config.what_to_count & COUNT_SRC_LOCAL_PREF)
                    fprintf(f, "%s%u", write_sep(sep, &count), pbgp->src_local_pref);
                ...
                if (config.what_to_count_2 & COUNT_SRC_ROA)
                    fprintf(f, "%s%s", write_sep(sep, &count), rpki_roa_print(pbgp->src_roa));
                if (config.what_to_count_2 & COUNT_DST_ROA)
                    fprintf(f, "%s%s", write_sep(sep, &count), rpki_roa_print(pbgp->dst_roa));

                if (config.what_to_count & COUNT_PEER_SRC_AS)
                    fprintf(f, "%s%u", write_sep(sep, &count), pbgp->peer_src_as);
                if (config.what_to_count & COUNT_PEER_DST_AS)
                    fprintf(f, "%s%u", write_sep(sep, &count), pbgp->peer_dst_as);
                ...
                if (config.what_to_count & COUNT_OUT_IFACE)
                    fprintf(f, "%s%u", write_sep(sep, &count), data->ifindex_out);
                ...
                if (config.what_to_count_2 & COUNT_MPLS_PW_ID)
                    fprintf(f, "%s%u", write_sep(sep, &count), pbgp->mpls_pw_id);
                ...
                if (config.what_to_count & (COUNT_SRC_PORT | COUNT_SUM_PORT))
                    fprintf(f, "%s%u", write_sep(sep, &count), data->src_port);
                ...
                if (config.what_to_count & COUNT_TCPFLAGS)
                    fprintf(f, "%s%u", write_sep(sep, &count), queue[j]->tcp_flags);
                ...
                if (config.what_to_count_2 & COUNT_SAMPLING_RATE)
                    fprintf(f, "%s%u", write_sep(sep, &count), data->sampling_rate);
                if (config.what_to_count_2 & COUNT_SAMPLING_DIRECTION)
                    fprintf(f, "%s%s", write_sep(sep, &count),
                ...
                if (config.what_to_count_2 & COUNT_POST_NAT_SRC_PORT)
                    fprintf(f, "%s%u", write_sep(sep, &count), pnat->post_nat_src_port);
                if (config.what_to_count_2 & COUNT_POST_NAT_DST_PORT)
                    fprintf(f, "%s%u", write_sep(sep, &count), pnat->post_nat_dst_port);
                if (config.what_to_count_2 & COUNT_NAT_EVENT)
                    fprintf(f, "%s%u", write_sep(sep, &count), pnat->nat_event);
                if (config.what_to_count_2 & COUNT_FW_EVENT) fprintf(f, "%s%u", write_sep(sep, &count), pnat->fw_event);
                if (config.what_to_count_2 & COUNT_FWD_STATUS)
                    fprintf(f, "%s%u", write_sep(sep, &count), pnat->fwd_status);

                if (config.what_to_count_2 & COUNT_MPLS_LABEL_TOP)
                    fprintf(f, "%s%u", write_sep(sep, &count), pmpls->mpls_label_top);
                if (config.what_to_count_2 & COUNT_MPLS_LABEL_BOTTOM)
                    fprintf(f, "%s%u", write_sep(sep, &count), pmpls->mpls_label_bottom);
                if (config.what_to_count_2 & COUNT_MPLS_LABEL_STACK) {
                    ...
                    if (label_stack_ptr) {
                        mpls_label_stack_to_str(label_stack, sizeof(label_stack), (u_int32_t *) label_stack_ptr,
                                                label_stack_len);
                    }
                ...
                if (config.what_to_count_2 & COUNT_TUNNEL_IP_TOS)
                    fprintf(f, "%s%u", write_sep(sep, &count), ptun->tunnel_tos);
                if (config.what_to_count_2 & COUNT_TUNNEL_SRC_PORT)
                    fprintf(f, "%s%u", write_sep(sep, &count), ptun->tunnel_src_port);
                if (config.what_to_count_2 & COUNT_TUNNEL_DST_PORT)
                    fprintf(f, "%s%u", write_sep(sep, &count), ptun->tunnel_dst_port);
                if (config.what_to_count_2 & COUNT_TUNNEL_TCPFLAGS)
                    fprintf(f, "%s%u", write_sep(sep, &count), queue[j]->tunnel_tcp_flags);
                ...
                if (config.what_to_count_2 & COUNT_EXPORT_PROTO_SEQNO)
                    fprintf(f, "%s%u", write_sep(sep, &count), data->export_proto_seqno);
                if (config.what_to_count_2 & COUNT_EXPORT_PROTO_VERSION)
                    fprintf(f, "%s%u", write_sep(sep, &count), data->export_proto_version);
                if (config.what_to_count_2 & COUNT_EXPORT_PROTO_SYSID)
                    fprintf(f, "%s%u", write_sep(sep, &count), data->export_proto_sysid);
                ...
                if (!is_event) {
                    ...
                    if (config.what_to_count & COUNT_FLOWS)
                        fprintf(f, "%s%" PRIu64 "", write_sep(sep, &count), queue[j]->flow_counter);
              ...
              write_avro_json_record_to_file(f, p_avro_value);
                ...
                if (config.debug)
                    Log(LOG_DEBUG,
                        "DEBUG ( %s/%s ): compose_avro_acct_data(): AVRO object not created due to missing --enable-avro\n",
                        config.name, config.type);
#endif
            }
            ...
            if (config.print_output & PRINT_OUTPUT_CUSTOM) {
                custom_print_plugin.print(config.what_to_count, config.what_to_count_2, queue[j]->flow_type,
                                          &queue[j]->primitives, pbgp, pnat, pmpls, ptun, pcust, pvlen,
                                          queue[j]->bytes_counter,
...
//  not being executed.
    ...
    if (config.sql_table) {
        ...
        if (config.print_latest_file) {
            if (!safe_action) {
                handle_dynname_internal_strings(tmpbuf, SRVBUFLEN, config.print_latest_file, &prim_ptrs,
                                                DYN_STR_PRINT_FILE);
...
void P_write_stats_header_csv(FILE *f, int is_event) {
    ...
    if (config.what_to_count_2 & COUNT_EXPORT_PROTO_VERSION)
        fprintf(f, "%sEXPORT_PROTO_VERSION", write_sep(sep, &count));
...
void
P_fprintf_csv_string(FILE *f, struct pkt_vlen_hdr_primitives *pvlen, pm_cfgreg_t wtc, char *sep, char *empty_string) {
